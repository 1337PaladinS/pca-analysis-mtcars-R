# pca-analysis-mtcars-R
PCA analysis of mtcars dataset in R via ggbiplot pkg
...
Principal Component Analysis (PCA) is a useful technique for exploratory
data analysis, allowing you to better visualize the variation present in
a dataset with many variables. It is particularly helpful in the case of
"wide" datasets, where you have many variables for each sample. In this
tutorial, you'll discover PCA in R.
...
- Then, you'll try a [simple PCA](#pca) with a simple and easy-to-understand data set.
- Next, you'll use the results of the previous section to [plot](#plot) your first PCA - Visualization is very important!
- You'll also see how you can get started on [interpreting the results](#interpret) of these visualizations and
- How to set the [graphical parameters](#graphparam) of your plots with the `ggbiplot` package!
- Of course, you want your visualizations to be as customized as possible, and that's why you'll also cover some ways of doing [additional customizations](#customize) to your plots!
- You'll also see how you can add a [new sample](#sample) to your plot and you'll end up [projecting a new sample](#project) onto the original PCA.
- Wrap-up
...
understand dataset. You will use the `mtcars` dataset, which is built
...
say: PC1 explains 63% of the total variance, which means that nearly two-thirds of the information in the dataset (9 variables) can be
...
I won't describe the results here in detail, but your PCA object
contains the following information:

-   The center point (`$center`), scaling (`$scale`), standard
    deviation(`sdev`) of each principal component
...
`ggbiplot` package, which offers a user-friendly and pretty function to
...
Don't forget to install the requirements at the top of this notebook.
...
see that the variables `hp`, `cyl`, and `disp` all contribute to PC1,
...
You'll provide an argument to `ggbiplot`: let's give it the `rownames`
of `mtcars` as `labels`. This will name each point with the name of the
car in question:
...
Now you can see which cars are similar to one another. For example, the
Maserati Bora, Ferrari Dino and Ford Pantera L all cluster together at
the top. This makes sense, as all of these are sports cars.
...
`groups` argument of ggbiplot. You'll also set the `ellipse` argument to
be `TRUE`, which will draw an ellipse around each group.
...
cars are characterized by high values for `cyl`, `disp`, and `wt`.
...
to plot these other components, by using the `choices` argument.
...
You don't see much here, but this isn't too surprising. PC3 and PC4
explain very small percentages of the total variation, so it would be
surprising if you found that they were very informative and separated
the groups or revealed apparent patterns.
...
`disp`, `wt`, and `mpg`. This provides us with some clues for future
...
You can also scale the samples (`obs.scale`) and the variables
(`var.scale`):

```{r}
ggbiplot(mtcars.pca,ellipse=TRUE, 
         obs.scale = 1, var.scale = 1, labels=rownames(mtcars), groups=mtcars.country)
```
...
As `ggbiplot` is based on the `ggplot` function, you can use the same
...
Okay, so let's say you want to add a new sample to your dataset. This is
a very special car, with stats unlike any other. It's super-powerful,
has a 60-cylinder engine, amazing fuel economy, no gears and is very
light. It's a "spacecar", from Jupiter.

Can you add it to your existing dataset and see where it places in
relation to the other cars?

You will add it to `mtcars`, creating `mtcarsplus`, then repeat
...
mtcarsplus.pca <-
  prcomp(mtcarsplus[, c(1:7, 10, 11)], center = TRUE, scale. = TRUE)
...
(for example, `cyl` and `mpg`), which contributed to PC1, separating
...
the mean of all the samples and allowing `spacecar` to skew this mean,
you get the mean of the rest of the samples and look at `spacecar` in
relation to this.
...
rotation of the PCA matrix to the `spacecar` sample. Then you can
`rbind()` the projected values for `spacecar` to the rest of the `pca$x`
matrix and pass this to `ggbiplot` as before:
...
You have learned the principles of PCA, how to create a biplot, how to
fine-tune that plot and have seen two different methods for adding
samples to a PCA analysis. Thanks for reading!
